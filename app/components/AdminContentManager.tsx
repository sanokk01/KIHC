...
const emptyBySection: Record<AdminSection, AdminContentRecord> = {
  ...
  events: { id: "", slug: "", title: "", heldAt: "", status: "draft", eventType: "세미나", thumbnailLabel: "", imageUrl: "", content: "" },
  ...
  institute: { id: "institute", title: "연구소 통합 안내 (신규)", impactStats: "", historyTimeline: "", expertRoster: "", annualReports: "" },
    ...
    <div className="admin-editor-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}>
      <section className="admin-editor" role="dialog" aria-modal="true" aria-labelledby="admin-editor-title">
        ...
        <div className="admin-editor-fields">
          ...
          {section === "events" && <><label><span>행사 구분</span><select value={value.eventType ?? "세미나"} onChange={(event) => field("eventType", event.target.value)}><option value="세미나">세미나</option><option value="간담회">간담회</option><option value="포럼">포럼</option><option value="컨퍼런스">컨퍼런스</option><option value="기타">기타</option></select></label><label><span>개최일</span><input value={value.heldAt ?? ""} onChange={(event) => field("heldAt", event.target.value)} placeholder="2026. 08. 15" /></label><label><span>썸네일 라벨</span><input value={value.thumbnailLabel ?? ""} onChange={(event) => field("thumbnailLabel", event.target.value)} placeholder="예: KIHC SEMINAR" /></label></>}
...
function AdminSingletonEditor({ section, initialRecords }: { section: "about" | "settings" | "institute"; initialRecords: AdminContentRecord[] }) {
  ...
  return <section className="admin-card admin-form-card"><div className="admin-storage-note"><strong>외부 DB 연결 대기</strong><span>{databasePendingMessage}</span></div><div className="admin-settings-form">{section === "about" ? <><ImageUploadField label="이사장 사진" value={record.chairmanImageUrl} onChange={(url) => field("chairmanImageUrl", url)} /><label><span>이사장 인사말</span><textarea rows={7} value={record.chairmanMessage ?? ""} onChange={(event) => field("chairmanMessage", event.target.value)} /></label><label><span>연구회 소개</span><textarea rows={7} value={record.organizationIntroduction ?? ""} onChange={(event) => field("organizationIntroduction", event.target.value)} /></label><ImageUploadField label="조직도 이미지" value={record.organizationImageUrl} onChange={(url) => field("organizationImageUrl", url)} /><label><span>설립 목적</span><textarea rows={4} value={record.purpose ?? ""} onChange={(event) => field("purpose", event.target.value)} /></label><label><span>비전</span><textarea rows={4} value={record.vision ?? ""} onChange={(event) => field("vision", event.target.value)} /></label></> : section === "institute" ? <><label className="wide"><span>임팩트 통계 (숫자로 보는 연구소)</span><textarea rows={5} value={record.impactStats ?? ""} onChange={(event) => field("impactStats", event.target.value)} placeholder="줄바꿈으로 구분. 예: EST. 2024 : 설립 연도" /></label><label className="wide"><span>연혁 / 타임라인</span><textarea rows={6} value={record.historyTimeline ?? ""} onChange={(event) => field("historyTimeline", event.target.value)} placeholder="줄바꿈으로 구분. 예: 2024 : KIHC 설립" /></label><label className="wide"><span>연구진 및 전문가</span><textarea rows={6} value={record.expertRoster ?? ""} onChange={(event) => field("expertRoster", event.target.value)} placeholder="줄바꿈으로 구분. 예: 홍길동 : 연구원장" /></label><label className="wide"><span>연차보고서 및 브로슈어</span><textarea rows={4} value={record.annualReports ?? ""} onChange={(event) => field("annualReports", event.target.value)} placeholder="줄바꿈으로 구분. 예: 2024 연차보고서 : /docs/2024.pdf" /></label></> : <><label><span>사이트명</span><input value={record.siteName ?? ""} onChange={(event) => field("siteName", event.target.value)} /></label><label><span>대표 이메일</span><input type="email" value={record.email ?? ""} onChange={(event) => field("email", event.target.value)} /></label><label className="wide"><span>추천 검색 키워드</span><input value={record.searchKeywords ?? ""} onChange={(event) => field("searchKeywords", event.target.value)} placeholder="쉼표(,)로 구분하여 입력. 예: 정책연구,미래전략" /></label><label className="wide"><span>Footer 기관 정보</span><textarea rows={5} value={record.footerInformation ?? ""} onChange={(event) => field("footerInformation", event.target.value)} /></label></>}</div><div className="admin-form-actions">{message && <p role="status">{message}</p>}<button type="button" onClick={save} disabled={!databaseConnected || saving}>{saving ? "저장 중..." : (databaseConnected ? "변경사항 저장" : "DB 연결 후 저장 가능")}</button></div></section>;
}
...
export function AdminContentManager({ section, initialRecords }: { section: AdminSection; initialRecords: AdminContentRecord[] }) {
  ...
  if (section === "about" || section === "settings" || section === "institute") return <AdminSingletonEditor section={section} initialRecords={initialRecords} />;
