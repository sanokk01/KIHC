...
/* eslint-disable @next/next/no-img-element -- upload previews use same-origin R2 media URLs. */
...
function ImageUploadField({ label, value, onChange }: { label: string; value?: string; onChange: (url: string) => void }) {
  ...
  return <label className="wide admin-upload-field"><span>{label}</span><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => choose(event.target.files?.[0])} disabled={uploading} />{uploading && <small>이미지 업로드 중...</small>}{value && <div className="admin-upload-result"><img src={value} alt="업로드 미리보기" /><button type="button" onClick={() => onChange("")}>이미지 제거</button></div>}{error && <small className="error">{error}</small>}<small>JPG, PNG, WEBP, GIF · 최대 6MB</small></label>;
}
  ...
  return (
    <div className="admin-editor-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}>
      <section className="admin-editor" role="dialog" aria-modal="true" aria-labelledby="admin-editor-title">
        ...
        <div className="admin-editor-actions"><button type="button" className="secondary" onClick={onCancel}>취소</button><button type="button" onClick={onSave} disabled={!value.title.trim() || saving}>{saving ? "저장 중..." : "DB에 저장"}</button></div>
      </section>
    </div>
  );
}
...
function AdminSingletonEditor({ section, initialRecords }: { section: "about" | "settings"; initialRecords: AdminContentRecord[] }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage("D1 데이터베이스에 저장되어 공개 홈페이지에 반영됩니다.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "저장하지 못했습니다."); }
    finally { setSaving(false); }
  };
  return <section className="admin-card admin-form-card"><div className="admin-storage-note connected"><strong>DB 연결됨</strong><span>저장 내용은 공동 관리자에게 공유되고 공개 홈페이지에 반영됩니다.</span></div><div className="admin-settings-form">{section === "about" ? <><ImageUploadField label="이사장 사진" value={record.chairmanImageUrl} onChange={(url) => field("chairmanImageUrl", url)} /><label><span>이사장 인사말</span><textarea rows={7} value={record.chairmanMessage ?? ""} onChange={(event) => field("chairmanMessage", event.target.value)} /></label><label><span>연구회 소개</span><textarea rows={7} value={record.organizationIntroduction ?? ""} onChange={(event) => field("organizationIntroduction", event.target.value)} /></label><ImageUploadField label="조직도 이미지" value={record.organizationImageUrl} onChange={(url) => field("organizationImageUrl", url)} /><label><span>설립 목적</span><textarea rows={4} value={record.purpose ?? ""} onChange={(event) => field("purpose", event.target.value)} /></label><label><span>비전</span><textarea rows={4} value={record.vision ?? ""} onChange={(event) => field("vision", event.target.value)} /></label></> : <><label><span>사이트명</span><input value={record.siteName ?? ""} onChange={(event) => field("siteName", event.target.value)} /></label><label><span>대표 이메일</span><input type="email" value={record.email ?? ""} onChange={(event) => field("email", event.target.value)} /></label><label className="wide"><span>Footer 기관 정보</span><textarea rows={5} value={record.footerInformation ?? ""} onChange={(event) => field("footerInformation", event.target.value)} /></label></>}</div><div className="admin-form-actions">{message && <p role="status">{message}</p>}<button type="button" onClick={save} disabled={saving}>{saving ? "저장 중..." : "변경사항 저장"}</button></div></section>;
}
...
export function AdminContentManager({ section, initialRecords }: { section: AdminSection; initialRecords: AdminContentRecord[] }) {
  ...
  const saveEditor = async () => {
    ...
    try {
      ...
      setEditor(null);
      setMessage("D1 데이터베이스에 저장되어 공동 관리자와 공개 홈페이지에 반영됩니다.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "저장하지 못했습니다."); }
    finally { setSaving(false); }
  };
