...
const emptyBySection: Record<AdminSection, AdminContentRecord> = {
  news: { id: "", slug: "", title: "", publishedAt: "", status: "draft", imageUrl: "", excerpt: "", content: "", category1: "공지사항" },
    ...
    <div className="admin-editor-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}>
      <section className="admin-editor" role="dialog" aria-modal="true" aria-labelledby="admin-editor-title">
        ...
        <div className="admin-editor-fields">
          ...
          {section === "news" && <><label><span>대분류 (Category)</span><select value={value.category1 ?? "공지사항"} onChange={(event) => field("category1", event.target.value)}><option value="공지사항">공지사항</option><option value="보도자료">보도자료</option><option value="NRC 동정">NRC 동정</option><option value="뉴스레터">뉴스레터</option><option value="채용정보">채용정보</option><option value="행사일정">행사일정</option></select></label><label className="wide"><span>요약</span><textarea rows={3} value={value.excerpt ?? ""} onChange={(event) => field("excerpt", event.target.value)} /></label><label className="wide"><span>본문</span><textarea rows={8} value={value.content ?? ""} onChange={(event) => field("content", event.target.value)} /></label></>}
