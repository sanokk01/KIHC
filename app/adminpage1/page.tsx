...
export default function AdminPage() {
  ...
  return (
    <AdminShell>
      <div className="admin-title"><div><p>DASHBOARD</p><h1>안녕하세요, 관리자님.</h1><span>KIHC 홈페이지의 콘텐츠 현황과 관리 메뉴를 확인하세요.</span></div><Link prefetch={false} href="/" target="_blank">사이트 보기 ↗</Link></div>
      <div className="admin-storage-note dashboard-note"><strong>개발용 관리자</strong><span>현재 편집 데이터는 이 브라우저에만 저장되며, 실제 공개 반영은 DB/API 연결 후 제공됩니다.</span></div>
      ...
      <section className="admin-card recent-card">
        <div className="admin-card-heading"><h2>최근 게시물</h2><span>공개 사이트의 현재 임시 데이터를 보여줍니다.</span></div>
