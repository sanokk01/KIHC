...
export default async function PromotionalMaterialsPage() {
  ...
  return <><SiteHeader /><main><PageHero eyebrow="Promotional Materials" title={dict.title} description={dict.desc} /><section className="section archive-section"><div className="container"><div className="listing-heading"><p>{dict.total}<strong>{materials.length}</strong>{dict.count}</p><span>{dict.note}</span></div><div className="archive-grid">{materials.map((item, index) => <Link className="archive-card" href={`/promotional-materials/${item.slug}`} key={item.id}><div className={`archive-thumbnail promotion-${index + 1}`}>{item.imageUrl ? <img src={item.imageUrl} alt="" /> : <><span>KIHC</span><strong>{item.thumbnailLabel}</strong><i>{String(index + 1).padStart(2, "0")}</i></>}</div><div className="archive-card-copy"><p>{item.category} · {item.publishedAt}</p><h2>{item.title}</h2><span>{dict.previewBtn}</span></div></Link>)}</div></div></section></main><Footer /></>;
}
